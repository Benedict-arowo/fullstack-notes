		...
		const items = Items.map((item) => {
			...
			const populateTags = item.tags.map((tag) => {
				return (
					<p
						key={tag}
						className="bg-blue-300 dark:bg-blue-800 px-3 py-1 rounded-full cursor-pointer"
					>
						{tag}
					</p>
				);
			...
			return (
				<div
					key={item._id}
					className="bg-white text-gray-500 px-4 w-full py-2 h-40 rounded grid gap-y-2 min-w-container-min"
				>
					{/* Header */}
					<div className="flex justify-between flex-row items-center">
						<p className="font-medium text-xl text-gray-600 truncate cursor-pointer">
							{item.title}
						</p>
						<div className="flex items-center gap-4">
							...
							<svg
								...
								strokeWidth={1.5}
								stroke="currentColor"
								className="w-6 hover:text-black duration-300 cursor-pointer"
					...
					<div className="flex justify-between text-gray-100 items-end flex-wrap">
						...
						<span className="bg-blue-300 dark:bg-blue-800 text-white hover:drop-shadow-lg cursor-pointer duration-500 px-3 py-1 rounded">
