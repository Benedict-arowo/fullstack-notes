...
const Sidebar = () => {
	...
	const FolderComponent = () => {
		const items = folderList.map((item) => {
			return (
				<div
					key={item._id}
					className="flex w-full transition-all duration-300 text-slate-700 justify-between cursor-pointer rounded-md px-4 py-1 bg-white dark:bg-blue-900 dark:text-slate-100"
				>
					<p>{item.name}</p>
	...
	return (
		...
		<aside
			id="mainSidebar"
			className={`h-full bg-blue-300 dark:bg-blue-800 flex flex-col relative pt-4 overflow-hidden ${sidebarStyles} duration-500`}
		>
			{/* Arrow */}
			<svg
				...
				viewBox="0 0 24 24"
				fill="currentColor"
				className={`w-6 text-white absolute -top-1 right-1 hover:translate-x-2 cursor-pointer duration-300 ${
					windowWidth < minWidth ? "hidden" : ""
			...
			{/* Header */}
			{sidebarToggled && (
				<h1 className="w-full text-center text-3xl text-blue-900 font-bold dark:text-gray-100">
					Notes
				</h1>
			)}
			<section className="mt-6 px-2">
				{sidebarToggled ? (
					<input
						...
						name="Search"
						id=""
						className="w-full rounded-full py-2 px-4 hover:scale-105 duration-500"
					...
					<svg
						...
						strokeWidth={1.5}
						stroke="currentColor"
						className="w-8 cursor-pointer text-white hover:scale-105 duration-500"
						onClick={showSearch}
					>
						<path
							strokeLinecap="round"
			...
			{/* Main Options */}
			<section
				className={`flex gap-2 mt-2 flex-col duration-300 text-md text-slate-200 ${
					sidebarToggled ? "p-4" : "p-0 mt-12"
				}`}
			>
				{/* View All Items */}
				<section
					className={`hover:text-black cursor-pointer flex w-full flex-row items-center gap-3 ${
						!sidebarToggled ? "justify-center" : ""
					}`}
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
			...
			<div
				...
				className="overlay duration-300 transition-all z-10"
			>
				...
				<div className="bg-blue-300 dark:bg-blue-800 rounded-md p-4 w-1/2">
					<input
						...
						id="folderSearch"
						placeholder="Search folders..."
						className="rounded-full px-6 py-4 text-2xl text-gray-600 w-full hover:drop-shadow-lg active:drop-shadow-lg transition-all duration-300"
					/>
					{controlledSearch.length > 0 && (
						<>
							{folderList && (
								<p className="text-sm dark:text-slate-200 text-gray-800 ml-5">
							...
							<div
								className={`${
									folderList ? "mt-8" : ""
								} flex gap-2 transition-all duration-300 min-h-fit flex-col`}
							>
			...
			<footer
				className={`absolute bottom-0 w-full flex justify-between px-2 py-1 items-center text-slate-200 ${
					sidebarToggled
						? "bg-blue-400 flex-row dark:bg-blue-900"
						: ""
				}`}
			>
				{sidebarToggled && (
					<p
						onClick={() => updateTheme()}
						className="font-bold cursor-pointer hover:text-blue-800 duration-500"
					>
				...
				<section
					className={`${
						sidebarToggled ? "" : "flex flex-col-reverse gap-2"
					}`}
				>
					<svg
						title="Settings"
						...
						strokeWidth={1.5}
						stroke="currentColor"
						className="w-8 hover:rotate-12 cursor-pointer duration-300"
					>
