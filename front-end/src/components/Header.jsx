...
const Header = () => {
    ...
    const updateSearch = async (event) => {
        ...
        const response = await customFetch({
            url: `notes?query=${newSearch}`, options: {
                headers: { 'Content-Type': 'application/json', 'Authorization': localStorage.getItem('token') },
            }
        })
        const data = await response.json()
    ...
    return (
        <>
            ...
            <section className='mx-8 mt-4 xs:mx-0'>
                ...
                <input autoComplete='off' autoCorrect='false' type="text" name="Search" id="" className='w-full rounded-full py-2 px-4 hover:drop-shadow-lg focus:drop-shadow-lg' placeholder='Search...' value={search} onChange={updateSearch} />
